...
/**
 * User Profile Page - Neobrutalismus Design
 * Lernfa.st Dashboard
 */
...
export default function ProfilePage() {
        ...
        if (response.ok) {
          ...
          if (data.success && data.user) {
            setFormData({
              ...
              learningGoals: data.user.learningGoals || undefined,
              experienceLevel: data.user.experienceLevel || "beginner",
              preferredDifficulty: data.user.preferredDifficulty || "medium",
              preferredCardCount: data.user.preferredCardCount || 5,
            });
          }
        }
      } catch (error) {
  ...
  const handleEmailChange = async () => {
    ...
    try {
      ...
      setEmailChangeMessage(
        `Bestätigungs-E-Mail wurde an ${session?.user?.email} gesendet! Bitte prüfe dein Postfach.`
      );
      setNewEmail("");
    } catch (error) {
